	...
	public BasicRes fillin(FillinReq req) {
		...
		try {
			...
			for (Question item : quList) { // 1(必) 2(必) 3(選)
				...
				for (Fillin fillin : fillinList) { // 1 2 3 3
					// id 不一致，跳過
...
//					needQIds.remove(fillin.getqId());
				...
				// 正常的情況: 問題是必填，然後又有回答，每跑完一題， qIds 就會包含有必填的問題 id
				// 因此若問題是必填但 qIds 又沒有包含該題的 id ，就表示沒有回答該題必填題
	...
	public FeedbackRes feedback(FeedbackReq req) {
		Optional<Quiz> op = quizDao.findById(req.getQuizId());
		if (op.isEmpty()) {
			return new FeedbackRes(ResMessage.QUIZ_NOT_FOUND.getCode(), // 方法回去的是 FeedbackRes
					ResMessage.QUIZ_NOT_FOUND.getMessage(), null);
		}
		Quiz quiz = op.get();
//		// 取得 List<Question> 的字串
//		String questionsStr = quiz.getQuestions();
//		// 將 questionsStr 轉成 List<Question>
		...
		try {
			...
			for (Response resItem : resList) {
				List<Fillin> fillList = mapper.readValue(resItem.getFillin(), new TypeReference<>() {
				...
				FeedbackDetail detail = new FeedbackDetail(quiz.getName(), quiz.getDescription(), //
						...
						resItem.getEmail(), resItem.getAge(), fillList);
	...
	public StatisticsRes statistics(FeedbackReq req) {
		...
		for (Response item : responseList) {
			...
			try {
				List<Fillin> fillList = mapper.readValue(fillinStr, new TypeReference<>() {
				});
				for (Fillin fillin : fillList) {
